...
const GAME_TAGLINE = import.meta.env.VITE_GAME_TAGLINE || 'ZK Garden Defense on Stellar';
      ...
      {!hasContract ? (
        <div className="card">
          ...
          <p style={{ color: 'var(--color-ink-muted)', marginTop: '1rem' }}>
            ...
            <code> VITE_HERBAL_MOONLIGHT_CONTRACT_ID</code> in the root <code>.env</code>.
          </p>
        </div>
      ) : !devReady ? (
        <div className="card">
          ...
          <p style={{ color: 'var(--color-ink-muted)', marginTop: '0.75rem' }}>
            Run <code>bun run setup</code> to generate dev wallets for Gardener and Creature.
          </p>
        </div>
      ) : !isConnected ? (
        <div className="card">
